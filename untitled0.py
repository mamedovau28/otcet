...
if mp_file and metki_file:
    ...
    # Если значение является строкой:
        ...
        # Если строка состоит только из цифр – это подкатегория, оставляем пустым
            if num_value.strip().isdigit():
                return ""
            else:
            # Если в строке есть и буквы, то это название категории (основная категория)
                return num_value
    # Если значение не строка (например, число) – считаем это подкатегорией
        return ""

# Применяем функцию к DataFrame для определения категории
    report_week_df['Категория'] = report_week_df.apply(determine_category, axis=1)
