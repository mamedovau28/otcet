...
# Дополнительный код начало
    ...
    tp_status = f"Отклонение от KPI прогноза: {((tp_target_calls - kpi_tp) / kpi_tp) * 100:.1f}%"
    oh_status = f"Отклонение от KPI прогноза: {((oh_primary_calls - kpi_oh) / kpi_oh) * 100:.1f}%"
